...
6. **Item rarity tiers** (engine-enforced, narrative-flavored): common / uncommon / rare / ancient / mythic / cursed / founder_relic. `ancient` = pre-Source archaeology find (e.g., carving fragment). `mythic` = Source-tied relic (ruler-cast artifact, monument shard). `cursed` = bad-omen item. `founder_relic` = pre-civilization founder-era item. Rare+ MUST tie to a Source-event (a breakthrough, a major storm, a cast terrain_shape) or a specific monument location. NOT `epic` or `legendary` — the engine schema rejects those; earlier Bible drafts cited D&D tiers in error.
