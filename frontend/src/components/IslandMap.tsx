...
export function IslandMap({ npcs, cycle }: Props) {
  ...
  useEffect(() => {
    ...
    // Ocean
    const oceanGrad = ctx.createRadialGradient(w / 2, h / 2, w * 0.15, w / 2, h / 2, w * 0.7);
    ...
    // NPCs - distribute around the island by role
    const perimeterRoles = new Set(['Scout', 'Scout-in-Training', 'Watcher', 'Runner']);
    const foragerRoles = new Set(['Forager', 'Fisher']);
    const centerRoles = new Set([
      'Organizer', 'Carpenter', 'Toolmaker', 'Potter', 'Hauler',
      'Healer', 'Field Planner', 'Inland Marker', 'Prospector',
    ]);

    npcs.filter((n) => n.alive).forEach((npc, idx) => {
      let x = cx, y = cy;
      const angle = (idx * 137.5) * (Math.PI / 180); // golden-angle spread
      if (perimeterRoles.has(npc.role)) {
        x = cx + Math.cos(angle) * r * 0.95;
        y = cy + Math.sin(angle) * r * 0.95;
      } else if (foragerRoles.has(npc.role)) {
        x = cx + Math.cos(angle) * r * 0.7;
        y = cy + Math.sin(angle) * r * 0.7;
      } else if (centerRoles.has(npc.role)) {
